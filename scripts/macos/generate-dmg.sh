...
if [ "$(uname -s)" != "Darwin" ]; then
	fail "DMG packaging only runs on macOS."
fi
...
[ "$SIGNING_IDENTITY" = "" ] || git -C "$SOURCE_ROOT" diff-index --quiet HEAD -- || fail "Signed release builds must not have unstaged changes!"
